...
const A: number[] = new Array<number>(10);
const B: number[] = new Array<number>(15);
const C: number[] = new Array<number>(25);
...
for (let i = 0; i < 10; i++) {
  A[i] = parseInt(readline.question(`Entre o ${String(i + 1).padStart(2, ' ')}o. elemento de [A] --> `) as string);
}
...
for (let i = 0; i < 15; i++) {
  B[i] = parseInt(readline.question(`Entre o ${String(i + 1).padStart(2, ' ')}o. elemento de [B] --> `) as string);
}
